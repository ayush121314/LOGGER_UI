...
| Pretty JSON ↔ Compact | Default expanded, syntax-highlighted JSON; a **Compact table** toggle switches to a real columnar table — sticky header, fixed `Time / Level / App` columns, the most common JSON fields auto-promoted to their own aligned columns (e.g. `requestId`, `durationMs`, `status`), zebra rows, and `Message` flexing to fill. Search highlight + level colours carry over |
